...
public static class AutoMapper
{
    ...
    internal static ICellReader GetDefaultSingleCellValueReader(MemberInfo member)
    {
    ...
    private static bool TryGetWellKnownMapper(Type memberType, FallbackStrategy emptyValueStrategy, out ICellValueMapper mapper, out IEmptyCellFallback emptyFallback, out IInvalidCellFallback invalidFallback)
    {
        ...
        IEmptyCellFallback GetEmptyFallback(FallbackStrategy strategy)
        {
            ...
            else if (strategy == FallbackStrategy.SetToDefaultValue || emptyValueStrategy == FallbackStrategy.SetToDefaultValue)
            {
        ...
        IInvalidCellFallback GetInvalidFallback(FallbackStrategy strategy)
        {
            if (strategy == FallbackStrategy.SetToDefaultValue)
            {
                return new FixedValueFallback(type.DefaultValue());
            }
        ...
        if (type == typeof(DateTime))
        {
            ...
            emptyFallback = GetEmptyFallback(FallbackStrategy.ThrowIfPrimitive);
            invalidFallback = GetInvalidFallback(FallbackStrategy.ThrowIfPrimitive);
        }
    ...
    internal static bool TryCreateGenericEnumerableMap<TElement>(MemberInfo member, FallbackStrategy emptyValueStrategy, out ManyToOneEnumerableMap<TElement> map)
    {
        ...
        if (!TryGetCreateElementsFactory<TElement>(member.MemberType(), out var factory))
        {
            map = null;
            return false;
        }
