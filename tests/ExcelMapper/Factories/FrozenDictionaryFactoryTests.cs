...
public class FrozenDictionaryFactoryTests
{
    [Fact]
    public void Begin_End_Success()
    {
        var factory = new FrozenDictionaryFactory<string, int>();

        // Begin.
        factory.Begin(1);
        var value = Assert.IsAssignableFrom<FrozenDictionary<string, int>>(factory.End());
        Assert.Equal([], value);

        // Begin again.
        factory.Begin(1);
        value = Assert.IsAssignableFrom<FrozenDictionary<string, int>>(factory.End());
        Assert.Equal([], value);
    }
    ...
    public void Set_Invoke_Success()
    {
        ...
        Assert.Equal(new Dictionary<string, int> { ["key1"] = 1 }, Assert.IsAssignableFrom<FrozenDictionary<string, int>>(factory.End()));
    }
    ...
    public void Set_InvokeOutOfRange_Success()
    {
        ...
        Assert.Equal(new Dictionary<string, int> { ["key1"] = 1, ["key2"] = 2 }, Assert.IsAssignableFrom<FrozenDictionary<string, int>>(factory.End()));
    }
